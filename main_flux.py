...
# PromptGuard's Flux training code
# 基于SD1.4训练代码，适配Flux模型的T5 text encoder
...
def main():
    parser = argparse.ArgumentParser(description="Flux model training script.")
    parser.add_argument(
        "--save_steps",
        type=int,
        default=100,
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to pretrained clip model or model identifier from huggingface.co/models (for image encoding).",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Pretrained T5 tokenizer name or path if not the same as model_name",
    )
    parser.add_argument(
        "--use_dual_encoder",
        action="store_true",
        help="Whether to use both CLIP and T5 text encoders (Flux default).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="text-inversion-model-flux",
    ...
    parser.add_argument(
        ...
        type=str,
        default="two_loss",
        choices=["two_loss", "three_loss"],
        help="Trainer to use: 'two_loss' or 'three_loss'."
    )
    ...
    # trainer
    import trainer_flux as t
    if args.trainer == "three_loss":
        trainer = t.ThreeLossFluxTrainer(args)
    elif args.trainer == "two_loss":
        trainer = t.TwoLossFluxTrainer(args)
    else:
        raise ValueError(f"Trainer {args.trainer} not found. Choose 'two_loss' or 'three_loss'.")
